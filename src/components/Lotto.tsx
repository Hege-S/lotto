...
export const Lotto = () => {
  ...
  const generateUniqueNumber = (): number => {
    ...
    if (numberList.some((lottoObject) => lottoObject.number === newNumber)) {
  ...
  return (
    <div>
      <div>
        <button className="addB" onClick={addMainNumber}>
          Nytt tall
        </button>
        <button className="addET" onClick={extraNumber}>
          Tilleggstall
        </button>
        <button className="resetB" onClick={resetButton}>
          -X-
        </button>
      </div>
      {
        <div className="lottoTallContainer">
          {numberList.map((lottoObject, index) => (
            <span key={lottoObject.id} className="fadeIn">
              <button
                className={`number ${
                  index >= numberList.length - 1 && numberList.length > 7
                    ? "tilleggsTall"
                    : "lotto"
                }`}
                onClick={() => {
